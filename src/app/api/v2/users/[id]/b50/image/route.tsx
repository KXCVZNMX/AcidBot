...
function getFonts() {
    ...
    cachedFonts = {
        mPlusMed: mPlusMedBuffer.buffer.slice(
            mPlusMedBuffer.byteOffset,
            mPlusMedBuffer.byteOffset + mPlusMedBuffer.byteLength
        ),
        mPlusReg: mPlusRegBuffer.buffer.slice(
            mPlusRegBuffer.byteOffset,
            mPlusRegBuffer.byteOffset + mPlusRegBuffer.byteLength
        ),
    };
    return cachedFonts;
}
...
export async function GET(
    ...
    console.log(
        `[b50Image] UPSTREAM FETCH COMPLETE: ${upstreamFetchEnd - upstreamFetchStart}ms (total: ${upstreamFetchEnd - funcStartTime}ms)`
    );
    ...
    console.log(
        `[b50Image] IMAGE RENDER COMPLETE: ${imageRenderEnd - imageRenderStart}ms (total: ${imageRenderEnd - funcStartTime}ms)`
    );
